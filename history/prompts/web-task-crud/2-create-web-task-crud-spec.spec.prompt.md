---
id: 2
title: Create web task CRUD spec
stage: spec
date: 2026-01-04
surface: agent
...
user: LENOVO
command: /sp.specify
labels: [spec,crud,authentication,jwt]
links:
...
You are an expert in Spec-Driven Development for full-stack web applications.

We have completed constitution-phase2.md for Phase 2: Full-Stack Todo Web App (Next.js + FastAPI + Neon + Better Auth + JWT).

Now create the main feature specification file: specs/features/web-task-crud.md

This spec covers:
- Multi-user task CRUD operations via REST API
- Authentication (signup/signin with Better Auth)
- JWT token issuance and verification
- User isolation (only own tasks visible/modifiable)
- Responsive frontend interface
- Persistent storage in Neon PostgreSQL

Structure it like Phase 1 spec:
...
Follow constitution-phase2.md strictly:
- Multi-user, JWT required
- Endpoints: /api/{user_id}/tasks (GET/POST/GET/{id}/PUT/{id}/DELETE/{id}/PATCH/{id}/complete)
- Better Auth on frontend, JWT verification on backend
- User_id in path for isolation
- Responsive UI with forms
- No extra features (only basic 5 CRUD + auth)

Use clear, professional language, tables, code blocks.
Output ONLY the complete web-task-crud.md content – start directly with heading.
...
- ✅ Impact: Created comprehensive feature specification for web task CRUD operations with authentication
- 🧪 Tests: No tests required for this specification document
- 📁 Files: Created specs/features/web-task-crud.md with 350+ lines of detailed requirements
- 🔁 Next prompts: Ready to proceed with planning phase using /sp.plan
- 🧠 Reflection: Specification follows constitution-phase2.md requirements and covers all requested functionality
